...
static void write_paths_opaque(FILE *fout, potrace_path_t *tree) {
  ...
  for (p=tree; p; p=p->sibling) {
    ...
    column = fprintf(fout, "<path fill=\"#%06x\" stroke=\"none\" d=\"", p->color);  //info.color
    newline = 1;
    lastop = 0;
    if (info.debug == 1) {
    ...
    for (q=p->childlist; q; q=q->sibling) {
      column = fprintf(fout, "<path fill=\"#%06x\" stroke=\"none\" d=\"", q->color);  //info.fillcolor
      newline = 1;
      lastop = 0;
      if (info.debug == 1) {
...
static void write_paths_transparent_rec(FILE *fout, potrace_path_t *tree) {
  ...
  for (p=tree; p; p=p->sibling) {
    ...
    if (info.grouping != 0) {
      column = fprintf(fout, "<path fill=\"#%06x\" stroke=\"none\" d=\"", p->color); //"<path d=\""
      newline = 1;
      lastop = 0;
    }
    if (info.debug == 1) {
...
int page_svg(FILE *fout, potrace_path_t *plist, imginfo_t *imginfo, potrace_path_s **arrayAppend, int arrayLen) {
  ...
  //append added by xudi
  for(int i=0;i<arrayLen;++i)
      write_paths_transparent(fout,arrayAppend[i]);
...
int page_gimp(FILE *fout, potrace_path_t *plist, imginfo_t *imginfo, potrace_path_s **arrayAppend, int arrayLen) {
  info.opaque = 0;
  info.grouping = 0;
  return page_svg(fout, plist, imginfo, arrayAppend, arrayLen); //append added by xudi
}
